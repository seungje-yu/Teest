...
//        java.util.Scanner sc = new java.util.Scanner(System.in);
        // 다른 패키지에 있는 클래스 사용시 패키지명 생략을 위해서 사용하는 구문 사용 (import)
        ...
        * int 범위를 초과한 값을 입력받으면 역시 InputMismatchException 발생
        * */
        ...
        * */
        ...
        /* 논리형 값 입력받기 */
        /* next Boolean() : 입력받은 값을 boolean형으로 변환 */
        ...
        /* 문자형 값 입력받기 */
        /*
        * 문자를 직접 입력 받는 기능은 제공하지 않음
        * 따라서 문자열을 입력 받고, 입력받은 문자에서 원하는 순번째 문자를 분리해서 사용
        * java.lang.String에 charAt(int index)를 사용
        *
        * index를 정수형으로 입력하면 문자열에서 해당 인덱스에 있는 한 문자를 문자형으로 반환해주는 기능
        * index는 0부터 시작하는 숫자체계이며 컴퓨터에서 주로 사용됨
        * 만약 존재하지 않는 인덱스를 입력하면 IndexOutOfBoundsException이 발생
        *
        * */
        sc.nextLine();     // 이건 에외케이스에 들어가는건데 뒤에서 다시 다룰예정
