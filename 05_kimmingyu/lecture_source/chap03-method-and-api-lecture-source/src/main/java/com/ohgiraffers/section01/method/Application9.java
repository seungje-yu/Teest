...
public class Application9 {
    ...
    public static void main(String[] args) {
        ...
        /*
        [static 메소드]
        - 다른 클래스에 작성한 static메소드인 경우 호출할 때 클래스명을 반드시 작성
        - 클래스명.메소드명();
         */
//        int max = maxNumberOf(first, second);  || 클래스명을 생략하면 에러 발생
       ...
       /*
        ...
        하지만 권장하지 않음
        이미 메로리에 로딩되어 있는 static 메소드는 여러 객체가 공유
        그 때 객체로 접근하면 인스턴스가 가진 값으로 공유된 값에 예상치 못하는 동작을 유발할 수 있기 때문에 사용을 제한해 달라는 경고
        시스템이 복잡할수록 객체의 상태를 추적하기 어려워 유지보수에 악영향
        */
