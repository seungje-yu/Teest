...
public class Application9 {
    ...
    public static void main(String[] args) {
        ...
        Calculator calc = new Calculator(); // 객체를 생성함
        int min = calc.minNumberOf(first, second);
        System.out.println("두 수 중 최소값은 : " + min);

        /* static 메소드
         * 다른 클래스에 작성한 static메소드인 경우 호출할 때 클래스명을 반드시 기술해야한다.
         * 클래스명.메소드명();
         */

//        int max = maxNuberOf(first, second); // 클래스명을 생략하면 에러 발생한다.
        int max = Calculator.maxNumberOf(first, second);
        System.out.println("두 수 중 최대값은 : " + max);

        // static으로 만들어진 메소드는 재사용 가능하므로 따로 객체를 생성해서 접근할 필요 없음
        /*
         ...
         * 사용을 제한해 달라는 경고이다. 시스템이 복잡해질수록 객체의 상태를 추적하기 어려워 유지보수에 악영향을 준다.
         */
        /*
