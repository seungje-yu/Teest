...
public class Application2 {
    public static void main(String[] args) {
        ...
        * */
        ...
        long lnum = 8;
//        int inum = lnum; //데이터 손실가능성을 컴파일러가 알려준다.(에러발생)
        int inum = (int) lnum; // 변경하려는 자료형을 명시하여 강제 형변환을 해야한다.

        short snum = (short) inum;

        double dnum = 8.0;
//        float fnum = dnum; //데이터 손실가능성을 컴파일러가 알려준다.(에러발생)
        float fnum = (float) dnum;
        System.out.println("fnum = " + fnum);

        //실수 -> 정수 변경시 강제 형변환이 필요하다.
        float fnum2 = 4.0f;
//        long lnum2 = fnum2; //소수점 자리이하 데이터 손실 가능성
        ...
        short snum2 = (short) ch; // 같은 2byte인데 왜 강제 변환을 해야하냐면 부호비트(sign bit)로 인한 값의 범위가 다르기 때문이다.

        int num1= 97;
        int num2=-97;
        char ch2 = (char) num1;
        char ch3 = (char) num2; // 음수도 강제형변환을 하면 대입할 수 있다. char은 0부터 시작이므로 음수는 안되지만 형변환에 한에서 가능 , char a= -97은 안됨

        // 논리혀은 강제형변환 규칙에서도 제외된다.
        boolean isTrue = true;
//        byte b = (byte) isTrue;
//        short s = (byte) isTrue;
//        int i = (byte) isTrue;
