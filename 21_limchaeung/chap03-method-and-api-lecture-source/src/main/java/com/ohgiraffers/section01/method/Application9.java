...
public class Application9 {
    ...
    * */
    ...
    int min = calc.minNumberOf(first, second); // min 자동 생성 방법?
        System.out.println("두 수 중 최소값은 : " + min);

        /*
        * static 메소드
        * 다른 클래스에 작성한 static메소드인 경우 호출할 때 클래스명을 반드시 기술해야 한다.
        * 클래스명.메소드명();
        *
        * */
//        int max = maxNumberOf(first, second); // 클래스명을 생략하면 에러 발생한다.
        int max = Calculator.maxNumberOf(first, second); // 클래스명을 생략하면 에러 발생한다.
        ...
        * 그 때 객체로 접근하게 되면 인스턴스가 가진 값을 공유된 값에 예상치 못하는 동작을 유발할 수 있기 때문에
        * 사용을 제한해달라는 경고이다. 시스템이 복잡해질수록 객체의 상태를 추적하기 어려워 유지보수에 악영향을 준다.
        * */
